...
def compute_fitness(genome):
  ...
  for gene in genome:
    cv2.rectangle(out, pt1=tuple(gene.pt1), pt2=tuple(gene.pt2), color=(int(gene.color[0]), int(gene.color[1]), int(gene.color[2])), thickness=-1, lineType=cv2.LINE_AA)

  # mean squared error
  fitness = 255. / compare_mse(img2, out)
