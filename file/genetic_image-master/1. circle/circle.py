...
import cv2, random, os # opencv / 랜덤 / 디렉터리 / 시스템 인자 설
...
filepath=('C:/Users/sis04/Desktop/file/genetic_image-master/1. circle/chr.jpg')
...
def compute_fitness(genome):
    ...
    # numpy.ones() - 1로 채워진 배열을 만든다 / 도화지를 만든다.
  out = np.ones((height, width, channels), dtype=np.uint8) * 255

    # [도화지에 유전자를 그린다] cv2.circle() - 원을 그린다 / thickness=-1 원을 색상으로 채운다
  for gene in genome:
  ...
  fitness = 255. / compare_mse(img, out) # 차이가 적을수록
  return fitness, out



# compute population - 세대들의 유전자들을 한꺼번에 돌연변이로 만드는 함수
def compute_population(g):
  genome = deepcopy(g)
  if len(genome) < 200: # 유전자의 개수가 200보다 작을 때 돌연변이가 될 확률을 각각 계산하면 빠름..
    for gene in genome:
    ...
    for gene in random.sample(genome, k=int(len(genome) * prob_mutation)):
      gene.mutate()
      # prob_mutation의 확률만큼 랜덤으로 유전자들을 뽑는다. 그 뽑은 유전자들을 전부 복제한다.

  # add gene - 동그라미를 추가하는 부분
...
if __name__ == '__main__':
  ...
  best_genome = [Gene() for _ in range(n_initial_genes)] # 인덱스 무시 -

  # 첫번째 유전자가 얼만큼 좋은지 평가합니다
  best_fitness, best_out = compute_fitness(best_genome) # fitness, out
  ...
  while True:
    try:
      ...
      # map() 함수는 for문과 같은 반복문을 사용하지 않아도 지정한 함수로 인자를
      # 여러번 전달해 그 결과를 list 형태로 뽑아 주는 유용한 함수이다.
      # [deepcopy(best_genome)] * n_population을 compute_population을 실행.

      # 1개씩 하는게 아니라 병렬처리를 통해 50개의 genome을 '한 세대당 유전자 그룹의 개수'만큼 다시 생성해서 results에 넣습니다.
    except KeyboardInterrupt:
      p.close()
      break

    results.append([best_fitness, best_genome, best_out])
    # a = [1, 2, 3] / a.append([5,6]) / [1, 2, 3, 4, [5, 6]]

    # 병렬처리 로직(깁밥말기) - zip(*iterator) -
    new_fitnesses, new_genomes, new_outs = zip(*results)

    # # x[0]==new_fitnesses - fitness 점수에따라 내림차순으로 정리한다.
