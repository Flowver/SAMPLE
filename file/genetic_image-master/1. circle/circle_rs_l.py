...
import cv2, random, os, sys # opencv / 랜덤 / 디렉터리
...
edges = cv2.Canny(img2, threshold1=100, threshold2=150)
edges = cv2.dilate(edges, np.ones((3,3), np.uint8), iterations = 1)
height, width, channels = img2.shape #bgr 형식으로 읽는다
...
def compute_fitness(genome):
  ...
  for gene in genome:
    cv2.circle(out, center=tuple(gene.center), radius=gene.radius, color=(int(gene.color[0]), int(gene.color[1]), int(gene.color[2])), thickness=1,lineType=cv2.LINE_AA)
